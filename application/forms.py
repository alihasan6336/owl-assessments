...
class RegisterForm(FlaskForm):
    ...
    work_email       = EmailField('Work Email', validators=[DataRequired(), Email(), Length(max=62), UniqueFieldValidator("Work Email")])
    department       = StringField('Department', validators=[Length(max=50)])
    phone_number     = StringField('Phone Number', validators=[DataRequired(), Length(max=15), NumericFieldValidator("Phone Number"), UniqueFieldValidator("Phone number")])
    country_code     = SelectField('Code +', validators=[DataRequired(), Length(max=5)], choices=counryCodes)
    ...
    postal_code      = StringField('Postal Code', validators=[Length(max=32), NumericFieldValidator("Postal code")])
